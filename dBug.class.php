 ...
 *
 * 
 * if the optional "forceType"   is given, the variable supplied to the 
...
class dBug {
	...
	function makeTDHeader($type,$header) {
		...
		$ocultar = ($type == 'array') ? " style=\"display:none;\"" : "";
		echo "<tr $str_d $ocultar >
	...
	function checkType($var) {
		switch(gettype($var)) {
			...
			case "boolean":
				...
				break;
			default:
                $this->varIsDefault($var);
				break;
		}
	}

	//if variable is a (String) Default type
	function varIsDefault($var) {
        $var = ($var=="") ? "[empty string]" : $var;
//        $var = ($var=="") ? "<b>".$this->getVariableName().":</b> [empty string]<br>" : "<b>".$this->getVariableName().":</b> ".$var."<br>";
//    	echo "<table cellspacing=0 border=0><tr>\n<td>".$var."</td>\n</tr>\n</table>\n";
        echo $this->makeTableHeader("string","string");
        echo $this->makeTDHeader("string","string");
        echo $var;
	...
	function varIsNULL() {
		echo "<b>".$this->getVariableName().":</b> "."NULL";
	}
	
	//if variable is a boolean type
	function varIsBoolean($var) {
		$var=($var==1) ? "<font color='green'>TRUE</font><br>" : "<font color='red'>FALSE</font><br>";
		echo $var;
	}

	//if variable is a SysObject type
	function varIsSysObject($var) {
		echo "
		<style>
		pre{
			color:red;
...
//		$var_ser = str_replace("(","<b>(</b>",$var_ser);
		print_r($var);
		echo "</pre><br>";
	}
	...
	function initJSandCSS() {
		echo <<<SCRIPTS
			...
			<style type="text/css">
				table.dBug_string,table.dBug_array,table.dBug_object,table.dBug_resource,table.dBug_resourceC,table.dBug_xml {
					font-family:monospace,Verdana, Arial, Helvetica, sans-serif; color:#000000; font-size:12px;
				}
				...
				/* array */
				table.dBug_string { background-color:#CC6600; }
				table.dBug_string td { background-color:#FFFFFF; }
				table.dBug_string td.dBug_stringHeader { background-color:#DD9900; }
				table.dBug_string td.dBug_stringKey { background-color:#EECC00; }
